...
async def get_available_models(current_admin: User = Depends(get_current_admin_user)):
    """
...
async def create_experiment(
    ...
    try:
        ...
        # Automatically start training after creation using Celery task
        training_status = "draft"
        task_id = None
        try:
            task_id = finetuning_service.start_training(experiment_id, config)
            logger.info(f"Auto-started training task {task_id} for experiment {experiment_id}")
        ...
        return ExperimentResponse(
            experiment={
                ...
                "base_model": model_name,
                "dataset_id": dataset_id,
                "task_id": task_id  # Include Celery task ID for WebSocket tracking
            },
            status="created",
...
async def start_experiment(
    ...
    Returns the Celery task ID for WebSocket tracking.
    """
    try:
        user_id = current_admin.username
        
        # Start training via Celery (returns task ID)
        task_id = finetuning_service.start_experiment_training(experiment_id, user_id)
...
async def stop_experiment(
    ...
    """
    Stop training for an experiment using Celery task.
    """
    try:
        user_id = current_admin.username
        
        # Stop training via Celery
        finetuning_service.stop_training(experiment_id)
...
async def get_user_datasets(current_admin: User = Depends(get_current_admin_user)):
    """
...
async def create_finetuning_dataset(
    ...
    try:
        ...
        websocket_url = f"/api/ws/qca-dataset/{task_id}"
...
@router.get("/datasets/create/{task_id}/status")
async def get_dataset_creation_status(
    task_id: str,
    current_admin: User = Depends(get_current_admin_user)
):
    """
    Get status of Q-C-A dataset creation task (polling endpoint)
    
    Use this endpoint for polling-based progress tracking, or use
    WebSocket /api/ws/qca-dataset/{task_id} for real-time updates.
    """
    try:
        import json
        import redis
        ...
        if progress_data:
            ...
            if result.state == 'PENDING':
                ...
                return {
                    "task_id": task_id,
                    "status": "FAILURE",
                    "data": {"message": str(result.result)}
                }
        ...
        logger.error(f"Error getting dataset creation status: {e}")
        raise HTTPException(status_code=500, detail=str(e))
